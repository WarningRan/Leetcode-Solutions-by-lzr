...
//    int length = 0;
    ...
    *length = 1;
    ...
    while (1){
        ...
        printf("%d \n", array[index]);
        index = is_popfront? index + 1: index - 1 ;
        if (temp_pop->left){
        ...
        if (isEmptyList(pop_list)){
            ...
            if(pop_list->num > 0) {
                ...
                *length = pop_list->num;
                ...
                pushback(return_array_list, array);
                is_popfront = is_popfront ? 0: 1;
                index = is_popfront? 0: *length - 1 ;
...
//    input->left->right = NULL;
    ...
    int** output = zigzagLevelOrder(input, returnSize, returnColumnSizes);
    printf("%d\n",*returnSize);
    for(int i = 0; i < *returnSize; i++)
    {
